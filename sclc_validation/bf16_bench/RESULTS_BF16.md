...
- **Dated addendum 2026-09-22 (reproducibility gap closed)**: an outside cross-reference (Andy) found that the per-gene stats CSVs behind the gate table above lived only on thinkstation1 under gitignored `runs/` -- checkable in prose but not independently recomputable from a clean clone. The raw `targeted_panel/stats/overexpress/*.csv` for all three 104M arms (`fp32_baseline`, `fp32_repeat`, `bf16`) are now committed under [`committed_run_stats/`](committed_run_stats/) (36 files total across this and the 316M arms, ~300 KB, well under the smallest threshold that would call for a derived table instead). Verified byte-identical to the live thinkstation1 copies at commit time (checksums in [`committed_run_stats/CHECKSUMS.sha256`](committed_run_stats/CHECKSUMS.sha256)) and independently re-run through the already-committed `compare_runs.py panel` against only the committed copies, reproducing this section's 0.9998 / 20/20 / 0.9933 / FAIL exactly.
